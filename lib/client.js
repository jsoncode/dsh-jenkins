...
const css = [
	...
	".dshj-check-list{display:flex;flex-wrap:wrap;gap:6px 16px;padding:4px 0;min-width:0}",
	...
	".dshj-maprow{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:10px 12px;border:1px solid var(--dsw-alias-border-l1,#eee);border-radius:10px;background:var(--dsw-alias-bg-layer-2,#fafafa)}",
	".dshj-maprow-main{min-width:0;display:flex;flex-direction:column;gap:2px}",
	".dshj-maprow-meta{font-size:12px;color:var(--dsw-alias-label-secondary,#888);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}",
	".dshj-env-badge{flex:none;display:inline-flex;align-items:center;justify-content:center;height:18px;padding:0 8px;border-radius:999px;font-size:11px;font-weight:600;letter-spacing:.02em;color:var(--dsw-alias-brand-primary,#1668e3);background:color-mix(in srgb,var(--dsw-alias-brand-primary,#1668e3) 12%,transparent)}",
	".dshj-project-modal{width:min(760px,100%);height:min(80vh,680px);min-height:420px}",
	".dshj-project-body{display:flex;flex-direction:column;gap:12px}",
	".dshj-project-json{display:flex;flex-direction:column;gap:8px;min-height:0;flex:1}",
	".dshj-project-json-text{flex:1;min-height:240px;font-family:ui-monospace,SFMono-Regular,Consolas,monospace;font-size:12px;line-height:1.55;white-space:pre;resize:vertical}",
	".dshj-project-jsonbar{display:flex;align-items:center;justify-content:space-between;gap:8px;flex:none}",
	".dshj-project-jsonbar .dshj-hint{margin:0}",
	".dshj-project-list{display:flex;flex-direction:column;gap:10px;min-width:0}",
	".dshj-project-item{border:1px solid var(--dsw-alias-border-l1,#eee);border-radius:10px;padding:10px 12px;background:var(--dsw-alias-bg-layer-2,#fafafa);display:flex;flex-direction:column;gap:8px;min-width:0}",
	".dshj-project-item-head{display:flex;align-items:center;gap:8px}",
	".dshj-project-name{font-weight:600}",
	".dshj-project-item-head>.dshj-btn{flex:none}",
	".dshj-project-env{display:grid;grid-template-columns:minmax(0,.8fr) minmax(0,1.1fr) minmax(0,1.1fr) auto 22px;align-items:center;gap:8px;min-width:0}",
	".dshj-project-env>.dshj-input{padding:5px 10px;font-size:12px}",
	".dshj-env-name-input{font-weight:600}",
	".dshj-target-del{width:22px;height:22px;font-size:12px}",
	".dshj-target-del:disabled{opacity:.35;cursor:not-allowed}",
	".dshj-project-item-ops{display:flex;align-items:center;gap:10px;flex:none}",
	".dshj-project-env-count{font-size:12px;color:var(--dsw-alias-label-secondary,#888)}",
	".dshj-param-toggle{flex:none;min-width:78px}",
	".dshj-param-box{display:flex;flex-direction:column;gap:6px;padding:8px 10px;border:1px dashed var(--dsw-alias-border-l2,#ddd);border-radius:8px;background:var(--dsw-alias-bg-base,#fff)}",
	".dshj-param-row{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1.2fr) 84px 24px;align-items:center;gap:6px;min-width:0}",
	".dshj-param-row>.dshj-input{padding:5px 10px;font-size:12px}",
	".dshj-param-type{padding:5px 22px 5px 8px;font-size:12px;border-radius:6px}",
	".dshj-param-del{width:22px;height:22px;font-size:12px}",
	".dshj-target-params-empty{font-size:12px;color:var(--dsw-alias-label-tertiary,#aaa)}",
	".dshj-project-overwrite{margin-right:auto}",
...
const serverHost = (u) => {
	...
	try {
		...
		return (stripped.indexOf("@") !== -1 ? stripped.slice(stripped.lastIndexOf("@") + 1) : stripped).split(":")[0].toLowerCase();
	}
};
/**
...
function matchServer(s, ref) {
	...
	if (r === "") return false;
	if (s.name === r || s.id === r) return true;
	if (normServerUrl(s.baseUrl) === normServerUrl(r)) return true;
	const host = serverHost(s.baseUrl);
	return host !== "" && host === serverHost(r);
}
//#endregion
...
/**
* dsh-jenkins —— 浏览器半边：本地偏好（localStorage 持久化的视图状态）。
*
* 目前只有「在菜单中显示」一个开关：关闭后宿主侧栏 footerAction 的
* 【Jenkins 配置】入口按钮渲染 null（不占位）。纯客户端视图偏好，走
* localStorage 即可（服务器配置 / 发布历史等数据仍由宿主半边持有）。
*
* 默认**开启**：侧栏入口是本插件既有的入口，升级后不应凭空消失；用户可在
* 宿主「设置 → Jenkins 配置」分区页，或插件弹框「配置」tab 顶部把它关掉。
* 两处渲染同一个 showInMenuStore，改一处另一处即时同步。
*/
/** localStorage 键。 */
const STORAGE_KEY = "dsh-jenkins.show-in-menu";
...
function ShowInMenuToggle() {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		...
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
			className: "dshj-pref-text",
			children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: "dshj-pref-label",
				children: t("showInMenu")
			}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: "dshj-pref-desc",
			...
			children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { className: "dshj-switch-knob" })
		})]
	});
}
//#endregion
...
*/
/** 集中式项目在缓存 / 历史里的分桶键前缀。 */
const PROJECT_CWD_PREFIX = "@project/";
/** 项目名 → 缓存 / 历史分桶键。 */
const projectCwd = (name) => PROJECT_CWD_PREFIX + name;
/** 是否为集中式项目的分桶键。 */
const isProjectCwd = (key) => typeof key === "string" && key.startsWith("@project/");
/** 从分桶键取项目名。 */
const projectNameOfCwd = (key) => isProjectCwd(key) ? key.slice(9) : "";
/** 从路径取「项目名」（工作区文件夹名，与宿主发现式配置的命名规则一致）。 */
const folderNameOf = (p) => {
	return String(p || "").replace(/[\\/]+$/, "").replace(/^.*[\\/]/, "");
};
/** 未命名环境的回退显示名（按下标：0=UAT、1=生产，其余「环境 N」）。 */
const envLabel = (index) => index === 0 ? t("envUat") : index === 1 ? t("envProd") : t("projectEnvName", { n: index + 1 });
/** 发布目标的显示名：优先 `name`，缺省按下标回退为 UAT / 生产 / 环境 N。 */
const targetLabel = (target, index) => {
	return String(target && target.name || "").trim() || envLabel(index);
};
/** 发布目标数组 → 发布表单配置元素。 */
const targetsToEntries = (targets) => (Array.isArray(targets) ? targets : []).map((target) => {
	const entry = {
...
function parseTargets(raw) {
	...
	for (const item of raw) {
		...
		const envRaw = record.environments && typeof record.environments === "object" && !Array.isArray(record.environments) ? record.environments : record.parameters && typeof record.parameters === "object" && !Array.isArray(record.parameters) ? record.parameters : {};
		const environments = {};
		for (const key of Object.keys(envRaw)) {
			const value = envRaw[key];
			environments[key] = value === null || value === void 0 ? "" : typeof value === "string" || typeof value === "number" || typeof value === "boolean" ? value : String(value);
		}
		const target = {
			job,
			server,
...
*/
const projectOpError = (res, fallback) => res && res.code === "unknown-op" ? t("projectsHostStale") : tErr(res, fallback);
//#endregion
...
function PublishTab({ initialCwd, sessionId, run, poller, storage, workspaceItems, onCountChange, onFooter, onOpenLog }) {
	const pathsKey = (0, react.useMemo)(() => [...new Set((Array.isArray(workspaceItems) ? workspaceItems : []).map((w) => w && typeof w.path === "string" ? w.path : "").filter((p) => p !== ""))], [workspaceItems]).join("\n");
	const [projects, setProjects] = (0, react.useState)({});
	const [project, setProject] = (0, react.useState)("");
	const autoPickedRef = (0, react.useRef)(false);
	const entries = (0, react.useMemo)(() => targetsToEntries(projects[project]), [projects, project]);
	const projectNames = (0, react.useMemo)(() => Object.keys(projects).sort((a, b) => a.localeCompare(b)), [projects]);
	const config = (0, react.useMemo)(() => entries.length > 0 ? { entries } : null, [entries]);
	const launchCwd = project ? projectCwd(project) : "";
	(0, react.useEffect)(() => {
		let alive = true;
		run(sessionId, {
			op: "mapLoad",
			cwds: pathsKey ? pathsKey.split("\n") : []
		}).then((r) => {
...
function LauncherContent({ cwd, sessionId, config, run, poller, storage, onCountChange, onFooter, onOpenLog }) {
	...
	const activeEntry = selectedServer ? entries.find((en) => matchServer(selectedServer, en.server)) || null : null;
	...
	(0, react.useEffect)(() => {
		...
		run(sessionId, {
			...
			} else setJobsError(tErr(r, t("jobsFailed")));
		}).catch((e) => {
			if (alive) {
				setJobsLoading(false);
	...
	(0, react.useEffect)(() => {
		const init = {};
		const entry = (activeEntry && activeEntry.job === selectedJobPath && selectedServer && matchServer(selectedServer, activeEntry.server) ? activeEntry : null) || (selectedServer ? entries.find((en) => en.job === selectedJobPath && matchServer(selectedServer, en.server)) || null : null);
		if (entry) {
	...
	const onSubmit = async () => {
		...
		try {
			...
			if (res && res.ok) {
				if (cwd) await storage.writeCache(sessionId, cwd, {
					serverId: selectedServerId,
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
		/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
			...
			children: [
				...
				configServerRefs.length > 0 && !configServerRefs.some((ref) => servers.some((s) => matchServer(s, ref))) ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					className: "dshj-config-source",
					children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						className: "dshj-warn",
						children: t("projectServerUnmatched")
					})
				}) : null
			]
		}),
			...
			detailLoading ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				...
				children: formKeys.map((k) => {
					...
					if (p && p.type === "boolean") control = /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
						...
						const picked = String(v === void 0 || v === null ? "" : v).split(delim).map((s) => s.trim()).filter((s) => s !== "");
						control = /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							className: "dshj-check-list",
							children: (p.choices || []).map((c) => {
								...
								return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
									...
									}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: String(c) })]
								}, String(c));
							})
						});
					...
					return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							p && p.dynamic ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
								className: "dshj-form-desc dshj-warn",
								children: t("paramDynamicHint")
							}) : null
						]
					}, k);
				})
...
//#endregion
//#region src/client/components/ProjectMapModal.tsx
/**
...
const toMap = (drafts) => {
	...
	for (const draft of drafts) {
		...
		if (!name) continue;
		map[name] = draft.targets.filter((target) => target.job.trim() || target.server.trim()).map((target) => {
			const environments = {};
...
function validate(drafts) {
	...
	for (let i = 0; i < drafts.length; i += 1) {
		...
		for (let j = 0; j < draft.targets.length; j += 1) {
			...
			if (!target.job.trim() || !target.server.trim()) return t("projectTargetIncomplete", {
				name,
				n: String(target.name || "").trim() || envLabel(j)
			});
		}
	}
	return "";
}
function ProjectMapModal({ run, sessionId, servers, workspaces, mapPath, initial, onSaved, onClose }) {
	const [drafts, setDrafts] = (0, react.useState)(() => toDrafts(initial));
	const [mode, setMode] = (0, react.useState)("form");
	const [jsonText, setJsonText] = (0, react.useState)("");
	const [jsonError, setJsonError] = (0, react.useState)("");
	const [openParams, setOpenParams] = (0, react.useState)("");
	const [overwrite, setOverwrite] = (0, react.useState)(false);
	const [busy, setBusy] = (0, react.useState)(false);
	const [error, setError] = (0, react.useState)("");
	const [note, setNote] = (0, react.useState)("");
	const serverOptions = (0, react.useMemo)(() => (Array.isArray(servers) ? servers : []).map((s) => ({
		id: s.baseUrl,
		label: s.name + (s.baseUrl && s.name !== s.baseUrl ? " · " + s.baseUrl : "")
	})), [servers]);
	const patchProject = (index, patch) => setDrafts((prev) => prev.map((item, i) => i === index ? {
		...item,
		...patch
	} : item));
	const patchTarget = (index, targetIndex, patch) => setDrafts((prev) => prev.map((item, i) => i === index ? {
		...item,
		targets: item.targets.map((target, j) => j === targetIndex ? {
			...target,
			...patch
		} : target)
	} : item));
	const addProject = () => setDrafts((prev) => prev.concat([{
		name: "",
		targets: [{
			name: "",
			job: "",
	...
	const addTarget = (index) => setDrafts((prev) => prev.map((item, i) => i === index ? {
		...item,
		targets: item.targets.concat([{
			name: "",
			job: "",
			server: "",
			params: []
	...
	const removeTarget = (index, targetIndex) => setDrafts((prev) => prev.map((item, i) => i === index ? {
		...item,
		targets: item.targets.filter((_, j) => j !== targetIndex)
	} : item));
	const addParam = (index, targetIndex) => setDrafts((prev) => prev.map((item, i) => i === index ? {
		...item,
		targets: item.targets.map((target, j) => j === targetIndex ? {
			...target,
			params: target.params.concat([{
				key: "",
				value: "",
				type: "string"
			}])
	...
	const patchParam = (index, targetIndex, paramIndex, patch) => setDrafts((prev) => prev.map((item, i) => i === index ? {
		...
		targets: item.targets.map((target, j) => j === targetIndex ? {
			...target,
			params: target.params.map((p, k) => k === paramIndex ? {
				...p,
				...patch
			} : p)
		} : target)
	...
	const removeParam = (index, targetIndex, paramIndex) => setDrafts((prev) => prev.map((item, i) => i === index ? {
		...
		targets: item.targets.map((target, j) => j === targetIndex ? {
			...target,
			params: target.params.filter((_, k) => k !== paramIndex)
		} : target)
	} : item));
	...
	const save = async () => {
		...
		try {
			...
			if (res && res.ok) {
				...
				onClose();
			} else setError(projectOpError(res, t("saveFailed")));
		} catch (e) {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(ModalPortal, {
		...
		children: [
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						...
						className: "dshj-modal-sub",
						children: mapPath || "dsh-jenkins-map.json"
					})] }),
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: "dshj-modal-body dshj-project-body",
				children: [
					mode === "json" ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: "dshj-project-json",
						children: [
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								className: "dshj-project-jsonbar",
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									className: "dshj-hint",
									children: t("projectJsonHint")
								}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									type: "button",
									className: "dshj-btn dshj-btn-small",
									onClick: applyJson,
									children: t("applyJsonBtn")
								})]
							}),
							jsonError ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
								className: "dshj-err",
								children: t("configParseFailed") + "：" + jsonError
							}) : null
						]
					}) : /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: "dshj-project-list",
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("datalist", {
								id: "dshj-project-servers",
								children: serverOptions.map((option) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
									value: option.id,
							...
							drafts.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
								className: "dshj-empty",
								children: t("projectsEmpty")
							}) : null,
							drafts.map((draft, index) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								className: "dshj-project-item",
								children: [
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										className: "dshj-project-item-head",
										children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
										...
										return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
											...
											children: [
												...
												/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
													...
													children: t("paramCount", { n: target.params.length })
												}),
												/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
													type: "button",
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										className: "dshj-project-item-ops",
										children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
											type: "button",
											className: "dshj-btn dshj-btn-small",
											onClick: () => addTarget(index),
											children: t("addTargetBtn")
										}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
											className: "dshj-project-env-count",
											children: t("envCount", { n: draft.targets.length })
										})]
									}),
									draft.targets.map((target, targetIndex) => {
										if (openParams !== index + ":" + targetIndex) return null;
										return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
											className: "dshj-param-box",
											children: [
												target.params.map((param, paramIndex) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
													className: "dshj-param-row",
													children: [
														/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
															className: "dshj-input",
															value: param.key,
														...
														/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("select", {
															...
															value: param.type,
															onChange: (e) => patchParam(index, targetIndex, paramIndex, { type: e.target.value }),
															children: [
																/* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
																	value: "string",
																	children: t("paramTypeText")
																}),
																/* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
																	value: "number",
																	children: t("paramTypeNumber")
																}),
																/* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
																	value: "boolean",
																	children: t("paramTypeBool")
																})
															]
														}),
														/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
															type: "button",
															className: "dshj-btn-icon dshj-param-del",
															title: t("deleteBtn"),
															onClick: () => removeParam(index, targetIndex, paramIndex),
															children: "✕"
														})
													]
												}, paramIndex)),
												target.params.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
													className: "dshj-target-params-empty",
													children: t("paramNone")
												}) : null,
												/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
													type: "button",
													className: "dshj-btn dshj-btn-small",
													onClick: () => addParam(index, targetIndex),
													children: t("addParamBtn")
												})
											]
										}, "p" + targetIndex);
									})
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
						className: "dshj-check dshj-project-overwrite",
						title: t("discoverOverwriteHint"),
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
							type: "checkbox",
							checked: overwrite,
							onChange: (e) => setOverwrite(e.target.checked)
						}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: t("discoverOverwrite") })]
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
						type: "button",
						className: "dshj-link-btn",
						...
						children: busy ? t("saving") : t("discoverBtn")
					}),
					mode === "form" ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
						type: "button",
						className: "dshj-btn",
						onClick: addProject,
						children: t("addProject")
					}) : null,
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
						type: "button",
						className: "dshj-btn",
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
						...
						disabled: busy,
						onClick: () => void save(),
						children: busy ? t("saving") : t("saveBtn")
					})
				]
			})
		]
...
function TemplateSection({ run, sessionId, cwd, workspaces }) {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		...
		children: [
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: "dshj-hint",
				children: t("templateCentralHint")
			}),
...
function SettingsPage({ run, sessionId, cwd, workspaceItems, onCountChange }) {
	...
	const [projectMap, setProjectMap] = (0, react.useState)({});
	const [mapPath, setMapPath] = (0, react.useState)("");
	const [mapError, setMapError] = (0, react.useState)("");
	const [mapOpen, setMapOpen] = (0, react.useState)(false);
	const workspaces = (0, react.useMemo)(() => [...new Set((Array.isArray(workspaceItems) ? workspaceItems : []).map((w) => w && typeof w.path === "string" ? w.path : "").filter((p) => p !== ""))], [workspaceItems]);
	...
	const loadMap = (0, react.useCallback)(() => {
		run(sessionId, {
			op: "mapLoad",
			cwds: workspacesKey ? workspacesKey.split("\n") : []
		}).then((r) => {
			if (r && r.ok) {
				setProjectMap(sanitizeProjects(r.map));
				setMapPath(String(r.path || ""));
				setMapError("");
			} else setMapError(projectOpError(r, t("loading")));
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		...
		children: [
			...
			mapOpen ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ProjectMapModal, {
				run,
				sessionId,
				servers,
				workspaces,
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", { className: "dshj-divider" }),
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: "dshj-maprow",
				title: t("projectsHint"),
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					className: "dshj-maprow-main",
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						className: "dshj-title",
						children: t("projectsTitle")
					}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						className: "dshj-maprow-meta",
						children: "dsh-jenkins-map.json · " + t("projectCount", { n: projectNames.length })
					})]
				}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
					type: "button",
					className: "dshj-btn dshj-btn-small",
			...
			mapError ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: "dshj-err",
				children: mapError
			}) : null
		]
...
function HistoryTab({ cwd, sessionId, run, poller, storage, onCountChange, onFooter, logTarget: logTargetProp, onLogTargetChange }) {
	...
	const cwdLabel = (key) => isProjectCwd(key) ? t("projectConfigTag") + "：" + projectNameOfCwd(key) : key;
		...
		confirmClear ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(ModalPortal, {
			...
			children: [
				...
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					...
					children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						className: "dshj-empty",
						children: filter === "all" ? t("confirmClearAll") : t("confirmClearCwd", { path: cwdLabel(filter) })
					})
				}),
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					className: "dshj-modal-footer",
...
* - 配置：原设置页内容（服务器管理：增删改查 / 测试连接 / 配置模板弹框）+
*   集中式「项目配置」区（项目名 → 发布目标数组，供发布 tab 一键选择）；
