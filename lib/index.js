...
function apply(ctx, config) {
	...
	const migrateLegacy = async (scope) => {
		...
		try {
			...
			legacyServers = Array.isArray(parsed) ? parsed : [];
		} catch {}
		try {
			const parsed = parseLegacyJson(value && value.cacheJson, "{}");
			legacyCache = parsed && typeof parsed === "object" ? parsed : {};
		} catch {}
		if (!(legacyServers.length > 0 || Object.keys(legacyCache).length > 0)) return;
	...
	storeReady = (async () => {
		try {
			...
			if (settings !== void 0) {
				let scope = null;
				try {
					scope = settings.register(settingsNamespace("dsh-jenkins"), LegacySettingsSchema, { base: {
