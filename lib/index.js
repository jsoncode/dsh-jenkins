...
async function runOp(deps, req) {
	...
	if (op === "footerOrderGet") return {
		ok: true,
		footerOrder: deps.readFooterOrder()
	};
	if (op === "footerOrderSet") {
		const v = String(req.footerOrder || "");
		if (v !== "front" && v !== "back") return {
			ok: false,
			code: "footer-order-invalid",
			error: "footerOrder must be \"front\" or \"back\""
		};
...
function apply(ctx, config) {
	...
	let scope = null;
	if (settings !== void 0) scope = settings.register(settingsNamespace("dsh-jenkins"), JenkinsSettingsSchema, { base: {
	...
	const readServers = () => {
		...
		} else raw = JSON.stringify(config.servers || []);
		try {
	...
	if (commands !== void 0) commands.register({
		name: "dsh-jenkins",
		description: "Jenkins CLI：管理服务器配置并触发/查询构建（设置界面/工作区入口调用）。Manage Jenkins servers and trigger/query builds (used by the settings UI and workspace entry). 参数为 JSON：{ \"op\": \"list|save|delete|test|jobs|jobDetail|trigger|queueStatus|buildStatus|buildLog|cancel|cacheGet|cacheSet|footerOrderGet|footerOrderSet|workspaceConfig|workspaceTrigger\", ... }。",
		input: { hint: "{\"op\":\"list\"}" },
		recordInput: true,
