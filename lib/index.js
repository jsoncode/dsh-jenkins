...
//#endregion
//#region src/host/projects.ts
/** 环境参数表：仅接受 string / number / boolean 三种标量值。 */
function normalizeEnvironments(raw, label) {
	if (raw === null || raw === void 0) return {};
	if (typeof raw !== "object" || Array.isArray(raw)) throw new Error(label + " 的 environments 需为对象（键值对）");
	const out = {};
...
*/
function normalizeProjectTarget(raw, label) {
	...
	const target = {
		job,
		server,
		environments: normalizeEnvironments(record.environments !== void 0 ? record.environments : record.parameters, label)
	};
	if (name) target.name = name;
	return target;
}
/** 归一化一个项目的发布目标数组（至少 1 项；数量不限）。 */
function normalizeProjectTargets(raw, projectName) {
...
function normalizeProjectMap(raw, opts) {
	if (!raw || typeof raw !== "object" || Array.isArray(raw)) throw new Error("集中配置需为对象：{ \"项目名\": [ 发布目标, ... ], ... }");
	const source = raw;
	const out = {};
...
function sanitizeProjectMap(raw) {
...
function projectNameFromFilename(filename) {
	const parts = String(filename || "").trim().replace(/\\/g, "/").split("/").filter((p) => p.length > 0);
	const stripped = (parts.length > 0 ? parts[parts.length - 1] : "").replace(/\.(json|js|cjs|mjs|ts)$/i, "").replace(/^dsh-jenkins[._-]?/i, "").trim();
	if (stripped) return stripped;
	return parts.length > 1 ? parts[parts.length - 2] : "imported";
}
/** 合并两份项目配置（incoming 覆盖同名项目，其余保留）。 */
function mergeProjectMaps(base, incoming) {
	const out = { ...base };
...
*/
function mergeMissingProjects(base, incoming) {
	const out = { ...base };
	const added = [];
...
function openStore(raw, key) {
	...
	return {
		...
		legacyProjects: sanitizeProjectMap(parsed.projects)
	};
}
/**
* 读取数据文件。
...
function saveStore(dir, store) {
	const next = writeChain$1.then(() => doSave$1(dir, store));
...
*/
const LOG_FILE = "dsh-jenkins.log";
const MESSAGE_MAX = 600;
const BODY_MAX = 800;
const STDERR_MAX = 400;
const clip = (value, max) => {
	const flat = (typeof value === "string" ? value : String(value ?? "")).replace(/[\r\n\t]+/g, " ");
...
function redactSecrets(text) {
	return text.replace(/Basic\s+[A-Za-z0-9+/=._-]{6,}/gi, "Basic ***").replace(/(https?:\/\/)[^/\s:@]+:[^/\s@]+@/gi, "$1***:***@").replace(/("?(?:token|password|passwd|apikey|api_key|api_token|secret)"?\s*[:=]\s*"?)([^"\s,}]{4,})/gi, "$1***").replace(/(crumb-value\s*=\s*")([^"]{4,})(")/gi, "$1***$3").replace(/("?(?:crumb|crumbRequestField|Jenkins-Crumb)"?\s*[:=]\s*"?)([^"\s,}]{4,})/gi, "$1***");
}
...
function render(rec) {
	...
	if (rec.curlExit !== void 0 && rec.curlExit !== null) entry.curlExit = rec.curlExit;
...
const codedError = (code, message, status) => {
...
const serverLabel = (server) => server.name ? `${server.name} <${normalizeBase(server.baseUrl)}>` : normalizeBase(server.baseUrl);
...
async function jenkinsRequest(ctx, server, path, opts) {
	...
	if (response.truncated === true) logFailure({
		stage: "http",
		op: opts?.op,
...
async function jenkinsJson(ctx, server, path, opts) {
	...
	if (r.status >= 300 && r.status < 400) {
		...
		const err = codedError("redirect", "服务器返回重定向（HTTP " + r.status + "）" + (location ? "：" + location : "") + "，请检查服务器地址是否为最终地址（如是否应使用 https://…）", r.status);
		logFailure({
			...evidence,
			code: "redirect",
			message: err.message,
			note: location ? "Location: " + location : void 0
		});
		throw err;
	}
	if (r.status >= 400) {
		...
		throw codedError("http-" + r.status, msg, r.status);
	}
	if (!r.body || !r.body.trim()) return null;
	try {
		return JSON.parse(r.body);
	} catch (e) {
		const err = codedError("parse-failed", "响应解析失败：" + e.message, r.status);
		logFailure({
			...evidence,
			code: "parse-failed",
			message: err.message,
...
async function getCrumb(ctx, server, op) {
	try {
		const r = await jenkinsRequest(ctx, server, "/crumbIssuer/api/json", op !== void 0 ? { op } : void 0);
...
*/
function splitChoices(raw, delimiter) {
	const text = String(raw || "");
	if (text.trim() === "") return [];
	return (text.indexOf("\n") !== -1 ? text.split(/\r?\n/) : text.split(delimiter && delimiter !== "" ? delimiter : ",")).map((s) => s.trim()).filter((s) => s !== "");
}
/** 从 JSON 定义里取参数默认值：`defaultValue` 或插件的 `defaultParameterValue.value`。 */
function defaultOf(d) {
	const direct = d.defaultValue;
	if (direct !== void 0 && direct !== null && typeof direct !== "object") return direct;
	const wrapped = d.defaultParameterValue;
...
function choicesOf(d, multi) {
	const raw = d.choices !== void 0 ? d.choices : d.value;
	if (Array.isArray(raw)) return raw.map((c) => String(c));
	if (typeof raw === "string") {
		const delimiter = typeof d.multiSelectDelimiter === "string" ? d.multiSelectDelimiter : void 0;
		const list = splitChoices(raw, multi ? delimiter : void 0);
...
function normalizeParamDef(d) {
	...
	if (cls.indexOf("BooleanParameterDefinition") !== -1) type = "boolean";
		...
		if (multiSelect) delimiter = typeof d.multiSelectDelimiter === "string" && d.multiSelectDelimiter !== "" ? d.multiSelectDelimiter : ",";
	}
	return {
		name,
		description: desc,
...
function extractParams(prop) {
	...
	if (holder === null) for (let i = 0; i < list.length; i++) {
		const x = list[i];
		if (x && Array.isArray(x.parameterDefinitions)) {
			holder = x;
			break;
		}
	}
...
*/
function parseBuildPageChoices(html) {
	const out = {};
	...
	while ((m = selectRe.exec(text)) !== null) {
		...
		out[name] = {
			choices,
			multiSelect: /\bmultiple\b/i.test(selectTag),
			defaultValue
		};
	}
	return out;
...
function normalizeConfig(raw) {
	...
	return {
		...
		entries: raw.map((e, i) => {
			...
			const parameters = record.environments && typeof record.environments === "object" && !Array.isArray(record.environments) ? record.environments : {};
			const name = String(record.name || "").trim();
			const target = {
				job,
				server,
...
*/
async function parseConfigFromContent(shell, filename, content) {
	const name = String(filename || "").trim().toLowerCase();
	const text = String(content || "").replace(/^\uFEFF/, "");
	if (!(name.endsWith(".json") || name.endsWith(".js") || name.endsWith(".cjs") || name.endsWith(".mjs") || name.endsWith(".ts"))) throw new Error("仅支持 dsh-jenkins 的 .json / .js / .ts 配置文件");
	let raw;
	if (name.endsWith(".json")) raw = JSON.parse(text);
	else {
		let ext;
		if (name.endsWith(".ts")) ext = ".ts";
		else if (name.endsWith(".mjs")) ext = ".mjs";
		else if (name.endsWith(".cjs")) ext = ".cjs";
		else ext = /(^|\n)\s*(import|export)[\s{*]/.test(text) ? ".mjs" : ".cjs";
		const b64 = Buffer.from(text, "utf8").toString("base64");
		const tsFlag = ext === ".ts" ? "--import tsx/esm " : "";
		const script = "import fs from 'node:fs';import os from 'node:os';import path from 'node:path';import {pathToFileURL as U} from 'node:url';const dir=fs.mkdtempSync(path.join(os.tmpdir(),'dshj-'));const file=path.join(dir,'config'+" + JSON.stringify(ext) + ");fs.writeFileSync(file,Buffer.from(process.argv[1],'base64'));import(U(file).href).then(m=>process.stdout.write(JSON.stringify(m.default??m))).catch(e=>{process.stderr.write(String((e&&e.message)||e));process.exit(1)}).finally(()=>{try{fs.rmSync(dir,{recursive:true,force:true})}catch{}})";
...
//#endregion
//#region src/host/project-map.ts
/**
...
*/
const MAP_FILE = "dsh-jenkins-map.json";
/** 项目配置文件绝对路径。 */
function mapFilePath(dir) {
	return join(dir, MAP_FILE);
}
/** 序列化为文件内容（2 空格缩进 + 末尾换行，便于人工编辑与 diff）。 */
function serializeProjectMap(map) {
	return JSON.stringify(map ?? {}, null, 2) + "\n";
}
/** 读取项目配置文件。不存在 → {}；损坏 → 备份 .bak 并返回 {}。 */
async function loadProjectMap(dir) {
...
function saveProjectMap(dir, map) {
...
function readCwds(req) {
	const raw = Array.isArray(req.cwds) ? req.cwds : [];
	const out = [];
...
*/
async function discoverFromWorkspaces(fsService, shell, cwds) {
	const discovered = {};
	const results = [];
	for (const cwd of cwds) {
		const name = cwd.replace(/[\\/]+$/, "").replace(/^.*[\\/]/, "") || projectNameFromFilename("");
		try {
...
*/
async function resolveDynamicChoices(ctx, server, segs, params, jobUrl) {
	if (params.filter((p) => p.type === "choice" && (p.choices === null || p.choices.length === 0)).length === 0) return params;
	let pageChoices = {};
	try {
		const res = await jenkinsRequest(ctx, server, String(jobUrl || "").trim() !== "" ? String(jobUrl).replace(/^https?:\/\/[^/]+/i, "").replace(/\/+$/, "") + "/build" : jobPath(segs) + "/build", {
	...
	return params.map((p) => {
		...
		if (page && page.choices.length > 0) return {
			...p,
...
async function runOp(deps, req) {
	...
	if (op === "configParseContent") {
		...
		if (!filename || !content.trim()) return {
			ok: false,
			code: "config-empty",
			error: "Missing config file content"
		};
		try {
			const shell = ctx.get("shell");
			...
			return {
				ok: true,
				config: await parseConfigFromContent(shell, filename, content)
			};
		} catch (e) {
	...
	if (op === "test") {
		...
		return {
			ok: true,
			version: data && data.version || headerValue(r.headers, "X-Jenkins") || "",
	...
	if (op === "jobs") {
		...
		let data;
		try {
			data = await jenkinsJson(ctx, s, "/api/json?tree=" + encodeURIComponent(tree), { op: "jobs" });
		} catch (e) {
			return {
				ok: false,
				code: errCodeOf(e),
		...
		if (data === null || typeof data !== "object") return {
			ok: false,
			code: "empty-response",
			error: "Jenkins 返回空响应，无法读取 Job 列表（请检查服务器地址、重定向与代理设置）"
		};
		if (data.jobs !== void 0 && !Array.isArray(data.jobs)) return {
			ok: false,
			code: "parse-failed",
			error: "响应格式异常：jobs 字段不是数组（代理 / 登录页可能返回了非 Jenkins 内容）"
		};
		const jobs = [];
	...
	if (op === "jobDetail") {
		...
		const data = await jenkinsJson(ctx, s, jobPath(segs) + "/api/json", { op: "jobDetail" });
	...
	if (op === "jobHistory") {
		...
		return {
			ok: true,
			builds: ((await jenkinsJson(ctx, s, jobPath(segs) + "/api/json?tree=builds%5Bnumber%2Ctimestamp%2Cresult%2Cbuilding%2Cduration%2Curl%2Cdescription%2CdisplayName%5D", { op: "jobHistory" })).builds || []).map((b) => ({
	...
	if (op === "queueStatus") {
		...
		const data = await jenkinsJson(ctx, s, "/queue/item/" + id + "/api/json", { op: "queueStatus" });
	...
	if (op === "buildStatus") {
		...
		try {
			const data = await jenkinsJson(ctx, s, path, { op: "buildStatus" });
			return {
				ok: true,
	...
	if (op === "buildLog") {
		...
		const res = await jenkinsRequest(ctx, s, jobPath(segs) + "/" + num + "/consoleText", { op: "buildLog" });
	...
	if (op === "pluginUpdateStatus") return {
		ok: true,
...
function apply(ctx, config) {
	...
	*/
		...
		try {
			...
			if (loaded !== null) {
				...
				if (legacyProjects && Object.keys(legacyProjects).length > 0) {
					const merged = mergeMissingProjects(await loadProjectMap(storeDir), legacyProjects);
	...
	const mapPath = () => mapFilePath(storeDir);
	const readMap = () => loadProjectMap(storeDir);
	const writeMap = (map) => saveProjectMap(storeDir, map);
			...
			webServer.register({
				...
				handler: async (req, res) => {
					...
					try {
						...
						if (payload === null || payload === void 0 || payload.ok !== true) logFailure({
							stage: "route",
							...
							sessionId: typeof request.sessionId === "string" ? request.sessionId : void 0,
							request: "POST /dsh-jenkins/api"
						});
