...
async function runOp(deps, req) {
	...
	if (op === "test") {
		...
		const r = await jenkinsRequest(ctx, {
			baseUrl,
...
const JenkinsSettingsSchema = Schema.object({ serversJson: Schema.string().default("[]") });
function apply(ctx, config) {
	if (ctx.get("shell") === void 0) return;
	const settings = ctx.get("settings");
	const commands = ctx.get("commands");
	let scope = null;
	if (settings !== void 0) scope = settings.register(settingsNamespace("dsh-jenkins"), JenkinsSettingsSchema, { base: { serversJson: JSON.stringify(config.servers || []) } });
	const readServers = () => {
		let raw = "[]";
		if (scope !== null) {
	...
	if (commands !== void 0) commands.register({
		name: "dsh-jenkins",
		description: "Jenkins CLI：管理服务器配置并触发/查询构建（设置界面/工作区入口调用）。Manage Jenkins servers and trigger/query builds (used by the settings UI and workspace entry). 参数为 JSON：{ \"op\": \"list|save|delete|test|jobs|jobDetail|trigger|queueStatus|buildStatus|workspaceConfig|workspaceTrigger\", ... }。",
		input: { hint: "{\"op\":\"list\"}" },
		recordInput: true,
