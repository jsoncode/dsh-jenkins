...
export function PublishTab({ initialCwd, sessionId, run, poller, storage, workspaceItems }) {
    ...
    return (_jsxs(_Fragment, { children: [_jsxs("div", { className: "dshj-server-field", children: [_jsx("label", { className: "dshj-server-label", children: t('projectField') }), _jsxs("button", { type: "button", className: 'dshj-picker' + (project ? '' : ' dshj-picker-empty'), disabled: paths.length === 0, onClick: () => { setProjectPickSearch(''); setProjectPickOpen(true); }, children: [_jsx("span", { className: "dshj-picker-value", children: project ? project : paths.length === 0 ? t('noWorkspacesHint') : t('projectPlaceholder') }), _jsx("span", { className: "dshj-picker-caret", children: "\u25BE" })] }), _jsx(PickerModal, { open: projectPickOpen, title: t('projectField'), search: projectPickSearch, setSearch: setProjectPickSearch, placeholder: t('projectPlaceholder'), options: paths
                            .filter((p) => p.toLowerCase().indexOf(projectPickSearch.toLowerCase()) !== -1)
                            .map((p) => ({ id: p, label: p })), selectedId: project || undefined, emptyText: paths.length === 0 ? t('noWorkspacesHint') : undefined, onSelect: (id) => { setProject(id); setProjectPickOpen(false); }, onClose: () => setProjectPickOpen(false) })] }), _jsx(LauncherContent, { cwd: project, sessionId: sessionId, config: config, run: run, poller: poller, storage: storage })] }));
}
