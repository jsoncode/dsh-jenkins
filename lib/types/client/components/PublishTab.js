...
import { fmtDur, t, tErr } from "../i18n.js";
import { matchServer } from "../storage.js";
import { folderNameOf, projectCwd, sanitizeProjects, targetLabel, targetsToEntries, } from "../projects.js";
...
export function PublishTab({ initialCwd, sessionId, run, poller, storage, workspaceItems, onCountChange, onFooter, onOpenLog }) {
    ...
    // 当前项目的发布目标（转成发布表单元素）
    const entries = useMemo(() => targetsToEntries(projects[project]), [projects, project]);
    // 项目名（排序）：下拉里「暂无」之后列出
    const projectNames = useMemo(() => Object.keys(projects).sort((a, b) => a.localeCompare(b)), [projects]);
    const config = useMemo(() => (entries.length > 0 ? { entries } : null), [entries]);
    ...
    return (_jsxs(_Fragment, { children: [_jsxs("div", { className: "dshj-server-field", children: [_jsx("label", { className: "dshj-server-label", children: t('projectField') }), _jsx("div", { className: "dshj-server-ctrl", children: _jsx(InlineSelect, { value: project, placeholder: projectNames.length === 0 ? t('noProjectsHint') : t('projectPlaceholder'), searchPlaceholder: t('pickerSearchPlaceholder'), options: [{ id: '', label: t('projectNone') }]
                                .concat(projectNames.map((name) => ({ id: name, label: name }))), onChange: (id) => setProject(id) }) })] }), _jsx(LauncherContent, { cwd: launchCwd, sessionId: sessionId, config: config, run: run, poller: poller, storage: storage, onCountChange: onCountChange, onFooter: onFooter, onOpenLog: onOpenLog })] }));
}
function LauncherContent({ cwd, sessionId, config, run, poller, storage, onCountChange, onFooter, onOpenLog }) {
    ...
    return (_jsxs(_Fragment, { children: [_jsxs("div", { className: "dshj-server-field", children: [_jsx("label", { className: "dshj-server-label", children: t('serverField') }), _jsxs("div", { className: "dshj-server-ctrl", children: [_jsx(InlineSelect, { value: selectedServerId, placeholder: t('noServersHint'), searchPlaceholder: t('pickerSearchPlaceholder'), options: serverPool.map((s) => ({ id: s.id, label: s.name })), disabled: !!runState || submitting || serverPool.length === 0, onChange: (id) => setSelectedServerId(id) }), _jsx("button", { type: "button", className: "dshj-btn dshj-btn-small dshj-server-side", title: t('goAdd'), disabled: !!runState || submitting, onClick: () => setAddServerOpen(true), children: t('goAdd') })] }), configServerRefs.length > 0 && !configServerRefs.some((ref) => servers.some((s) => matchServer(s, ref))) ? (_jsx("div", { className: "dshj-config-source", children: _jsx("span", { className: "dshj-warn", children: t('projectServerUnmatched') }) })) : null] }), _jsxs("div", { className: "dshj-server-field", children: [_jsx("label", { className: "dshj-server-label", children: t('jobField') }), _jsxs("div", { className: "dshj-server-ctrl", children: [_jsx(InlineSelect, { value: selectedJobPath, placeholder: !selectedServer ? t('jobPlaceholder')
                                ...
                                : formKeys.length === 0 ? _jsx("div", { className: "dshj-empty", children: t('noParams') })
                                    : (_jsx("div", { className: "dshj-form-grid", children: formKeys.map((k) => {
                                            ...
                                            if (p && p.type === 'boolean') {
                                                ...
                                                control = (_jsx("div", { className: "dshj-check-list", children: (p.choices || []).map((c) => {
                                                        const on = picked.indexOf(String(c)) !== -1;
                                                        return (_jsxs("label", { className: "dshj-check", children: [_jsx("input", { type: "checkbox", checked: on, onChange: () => set((on ? picked.filter((x) => x !== String(c)) : picked.concat([String(c)])).join(delim)) }), _jsx("span", { children: String(c) })] }, String(c)));
                                                    }) }));
                                            }
                                            ...
                                            return (_jsxs("div", { className: "dshj-form-field", children: [_jsx("label", { className: "dshj-form-label", title: k, children: k }), control, p && p.description && !descInControl ? _jsx("div", { className: "dshj-form-desc", children: p.description }) : null, p && p.dynamic ? _jsx("div", { className: "dshj-form-desc dshj-warn", children: t('paramDynamicHint') }) : null] }, k));
