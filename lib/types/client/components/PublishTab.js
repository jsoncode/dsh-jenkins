...
export function PublishTab({ initialCwd, sessionId, run, poller, storage, workspaceItems, onCountChange, onFooter, onOpenLog }) {
    ...
    return (_jsxs(_Fragment, { children: [_jsxs("div", { className: "dshj-server-field", children: [_jsx("label", { className: "dshj-server-label", children: t('projectField') }), _jsxs("div", { className: "dshj-server-ctrl", children: [_jsx(InlineSelect, { value: project, placeholder: paths.length === 0 ? t('noWorkspacesHint') : t('projectPlaceholder'), searchPlaceholder: t('pickerSearchPlaceholder'), options: [{ id: '', label: t('projectNone') }].concat(paths.map((p) => ({ id: p, label: p }))), onChange: (id) => { setProject(id); setFileConfig(null); setConfigError(''); } }), _jsx("input", { ref: fileInputRef, type: "file", accept: ".json,.js,.cjs,.mjs,.ts", className: "dshj-file-input", onChange: onPickFile }), _jsx("button", { type: "button", className: "dshj-btn dshj-btn-small dshj-server-side", title: t('selectConfigHint'), disabled: configLoading, onClick: () => fileInputRef.current && fileInputRef.current.click(), children: configLoading ? t('configPickLoading') : t('selectConfig') })] }), fileConfig ? (_jsxs("div", { className: "dshj-config-source", children: [_jsxs("span", { className: "dshj-config-source-text", children: [t('configSourceLabel'), "\uFF1A", _jsx("span", { className: "dshj-config-source-path", children: fileConfig.name }), " \u00B7 ", t('configTargetCount', { n: fileConfig.config.entries.length })] }), _jsx("button", { type: "button", className: "dshj-config-source-clear", title: t('configClear'), onClick: () => { setFileConfig(null); setConfigError(''); }, children: "\u2715" })] })) : configError ? (_jsx("div", { className: "dshj-config-source", children: _jsx("span", { className: "dshj-err", children: configError }) })) : null] }), _jsx(LauncherContent, { cwd: project, configCwd: configCwd, sessionId: sessionId, config: config, run: run, poller: poller, storage: storage, onCountChange: onCountChange, onFooter: onFooter, onOpenLog: onOpenLog })] }));
}
