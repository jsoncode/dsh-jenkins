...
export function PublishTab({ initialCwd, sessionId, run, poller, storage, workspaceItems, onCountChange, onFooter }) {
...
function LauncherContent({ cwd, sessionId, config, run, poller, storage, onCountChange, onFooter }) {
    ...
    const footerNode = useMemo(() => {
        if (runState) {
            return (_jsxs(_Fragment, { children: [_jsx("button", { type: "button", className: "dshj-btn", onClick: () => setRunState(null), children: t('backParams') }), runState.phase === 'done' ? (_jsx("button", { type: "button", className: "dshj-btn dshj-btn-primary", onClick: stableSubmit, children: t('rebuild') })) : null] }));
        }
        if (!selectedJobPath)
            return null;
        return (_jsxs(_Fragment, { children: [_jsx("button", { type: "button", className: "dshj-link-btn", disabled: submitting, onClick: () => setParamsOpen(true), children: t('viewParams') }), _jsx("button", { type: "button", className: "dshj-btn dshj-btn-primary", disabled: submitting, onClick: stableSubmit, children: submitting ? t('submitting') : t('submit') })] }));
                                    ...
                                    : formKeys.length === 0 ? _jsx("div", { className: "dshj-empty", children: t('noParams') })
                                        : (_jsx("div", { className: "dshj-form-grid", children: formKeys.map((k) => {
                                                ...
                                                if (p && p.type === 'boolean') {
                                                    ...
                                                    control = (_jsx(InlineSelect, { value: String(v), searchPlaceholder: t('pickerSearchPlaceholder'), options: (p.choices || []).map((c) => ({ id: String(c), label: String(c) })), onChange: (id) => set(id) }));
                                                }
