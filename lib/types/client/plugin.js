...
import { makeConfigModalStore, makeFooterOrderStore } from "./store.js";
        ...
        apply(ctx) {
            ...
            slots.inject(FOOTER_SLOT, () => {
                ...
                const register = () => {
                    ...
                    dispose = slots.register({ name: FOOTER_SLOT, id: FOOTER_ENTRY_ID, order }, (props) => (_jsx(FooterButton, { onOpen: () => configStore.open(true), reportSession: (s) => { if (s)
                ...
                return () => { unsubscribePolicy(); unsubscribeSlot(); if (dispose)
                    dispose(); };
            });
            // ─── 统一「Jenkins 配置」弹框（发布 / 配置 / 历史 三 tab）──────────
            slots.inject('shell.overlay', () => slots.register({ name: 'shell.overlay', id: 'dsh-jenkins-config', order: 100 }, (props) => (_jsx(JenkinsConfigModal, { run: run, poller: poller, storage: storage, useOpen: useConfigOpen, close: () => configStore.close(), footerOrderStore: footerOrderStore, useWorkspaces: props.useWorkspaces, useSessions: props.useSessions }))));
        },
    };
}
