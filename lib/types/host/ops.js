 ...
 */
import { errorCodeOf, extractParams, getCrumb, headerValue, jenkinsJson, jenkinsRequest, jobPath, jobSegments, normalizeBase, parseBuildPageChoices, } from "./jenkins.js";
import { loadWorkspaceConfig, parseConfigFromContent } from "./workspace-config.js";
import { mergeMissingProjects, mergeProjectMaps, normalizeProjectMap, projectNameFromFilename } from "./projects.js";
import { MAP_FILE } from "./project-map.js";
    ...
    for (const cwd of cwds) {
        ...
        try {
            ...
            discovered[name] = config.entries.map((en) => {
                const target = { job: en.job, server: en.server, environments: en.parameters || {} };
...
async function resolveDynamicChoices(ctx, server, segs, params, jobUrl) {
    ...
    try {
        ...
        if (res.status < 500 && res.body)
            pageChoices = parseBuildPageChoices(res.body);
    }
    catch { /* 构建页取不到：下面按降级处理 */ }
    return params.map((p) => {
        if (p.type !== 'choice' || (p.choices !== null && p.choices.length > 0))
            return p;
...
export async function runOp(deps, req) {
    ...
    if (op === 'configParseContent') {
        ...
        if (!filename || !content.trim())
            return { ok: false, code: 'config-empty', error: 'Missing config file content' };
        try {
            const shell = ctx.get('shell');
            if (shell === undefined)
                return { ok: false, code: 'fs-missing', error: 'shell service unavailable' };
    ...
    if (op === 'mapLoad') {
        ...
        const base = { ok: true, map, file: MAP_FILE, path: deps.mapPath(), added: [], results: [] };
        if (cwds.length === 0)
            return base;
        const fsService = ctx.get('fs');
        const shell = ctx.get('shell');
        ...
        if (fsService === undefined || shell === undefined)
            return { ...base, discoverError: 'fs-missing' };
    ...
    if (op === 'mapDiscover') {
        ...
        const cwds = readCwds(req);
        if (cwds.length === 0)
            return { ok: false, code: 'cwd-missing', error: 'Missing workspace paths' };
        const fsService = ctx.get('fs');
        const shell = ctx.get('shell');
        if (fsService === undefined || shell === undefined) {
            return { ok: false, code: 'fs-missing', error: 'fs/shell service unavailable' };
    ...
    if (op === 'jobHistory') {
        ...
        const data = await jenkinsJson(ctx, s, jobPath(segs) + '/api/json?tree=' + encodeURIComponent(tree), { op: 'jobHistory' });
