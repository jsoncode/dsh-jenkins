 ...
 */
export function normalizeProjectTarget(raw, label) {
    if (!raw || typeof raw !== 'object' || Array.isArray(raw))
        throw new Error(label + ' 需为对象');
    const record = raw;
    const job = String(record.job || '').trim();
    if (!job)
        throw new Error(label + ' 缺少 job（Jenkins 任务路径）');
    const server = String(record.server || '').trim();
    if (!server)
        throw new Error(label + ' 缺少 server（服务器名称 / id / 地址）');
    ...
    const target = { job, server, environments: normalizeEnvironments(env, label) };
    if (name)
        target.name = name;
    return target;
}
/** 归一化一个项目的发布目标数组（至少 1 项；数量不限）。 */
export function normalizeProjectTargets(raw, projectName) {
    if (!Array.isArray(raw))
        throw new Error('项目「' + projectName + '」的值需为数组（每个元素一个发布目标）');
    if (raw.length === 0)
        throw new Error('项目「' + projectName + '」至少需要一个发布目标');
    return raw.map((item, i) => normalizeProjectTarget(item, '项目「' + projectName + '」第 ' + (i + 1) + ' 个发布目标'));
}
/** 归一化整个 map（严格模式：任一项非法即抛错；allowEmpty 时接受空 map）。 */
export function normalizeProjectMap(raw, opts) {
    if (!raw || typeof raw !== 'object' || Array.isArray(raw)) {
    ...
    if (names.length === 0 && !(opts && opts.allowEmpty))
        throw new Error('集中配置不能为空');
...
export function sanitizeProjectMap(raw) {
    if (!raw || typeof raw !== 'object' || Array.isArray(raw))
        return {};
    const source = raw;
    const out = {};
 ...
 * 都没有则用 `imported`。
 */
export function projectNameFromFilename(filename) {
...
export function mergeProjectMaps(base, incoming) {
    const out = { ...base };
 ...
 */
export function mergeMissingProjects(base, incoming) {
    const out = { ...base };
    const added = [];
    for (const name of Object.keys(incoming)) {
        if (name in out)
