 ...
 */
export function parseCurlDump(stdout) {
    const statuses = [];
    let headers = '';
    let pos = 0;
    for (;;) {
    ...
    if (statuses.length === 0) {
        ...
        if (m !== null)
            return { statuses: [Number(m[1])], headers: stdout, body: '' };
    }
...
export function errorCodeOf(e) {
    ...
    if (err && typeof err.code === 'string' && err.code.length > 0) {
        if (err.code === 'http-401')
            return 'auth-failed';
        if (err.code === 'http-403')
            return 'forbidden';
        if (err.code === 'http-404')
            return 'not-found';
        if (err.code.startsWith('http-'))
    ...
    if (msg.indexOf('响应解析失败') !== -1)
        return 'parse-failed';
    if (msg.indexOf('响应过大') !== -1)
        return 'response-too-large';
    if (msg.indexOf('未取得 HTTP 响应') !== -1)
        return 'empty-response';
    if (msg.indexOf('重定向') !== -1)
        return 'redirect';
    if (msg.indexOf('subprocess 服务不可用') !== -1 || msg.indexOf('启动 curl 失败') !== -1)
...
export async function getCrumb(ctx, server, op) {
    try {
...
export function normalizeParamDef(d) {
    ...
    if (cls.indexOf('BooleanParameterDefinition') !== -1)
        ...
        if (/multi[_ -]?select|checkbox/i.test(kind))
            multiSelect = true;
...
export function extractParams(prop) {
    ...
    for (let i = 0; i < defs.length; i++) {
        ...
        if (def.name === '') {
            ...
            if (def.type !== 'divider' || def.description === '')
                continue;
            dividerIndex += 1;
 ...
 */
export function parseBuildPageChoices(html) {
    const out = {};
    ...
    while ((m = selectRe.exec(text)) !== null) {
        ...
        while ((om = optionRe.exec(body)) !== null) {
            ...
            if (value === '')
                continue;
            choices.push(value);
            if (/\bselected\b/i.test(attrs) && defaultValue === '')
                defaultValue = value;
        }
        if (choices.length === 0)
            continue;
