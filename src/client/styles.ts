...
export const css = [
  ...
  // 多选参数（Extended Choice multiSelect / uno-choice MultiSelect）：勾选列表，可换行
  ...
  // 模板弹框「项目」选择行：label + 可搜索下拉（保存前明确落盘位置），随 flex 列布局固定高度
  ...
  // 「配置」tab 的项目配置行：一行摘要（标题 + map 文件名/项目数）+ 右侧「编辑 map」按钮
  ...
  // 环境徽标：UAT / 生产 / 环境 N（项目配置弹框的表单里用；发布 tab 的环境名直接写进服务器下拉标签）
  ...
  // 表单 tab：项目紧凑列表（项目名一行 + 每个环境一行 + 参数按需展开）
  ...
  // 环境行：环境名（选填） / Job / 服务器 / 参数按钮 / 删除（数量不限，逐行可删）
