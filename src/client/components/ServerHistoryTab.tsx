...
/** 服务器返回的单条构建记录（op jobHistory 的 builds 元素）。 */
...
const stripHtml = (s: string): string => String(s || '')
  .replace(/<br\s*\/?\s*>/gi, ' ')
  .replace(/<[^>]+>/g, '')
  .replace(/\s+/g, ' ')
  .trim()
...
const displaySuffix = (name: string, number: number | null): string => {
...
const nameText = (b: ServerBuild): string => {
  const num = b.number != null ? '#' + b.number : '#?'
  const suffix = displaySuffix(b.displayName, b.number)
  return suffix ? num + ' - ' + suffix : num
}

export function ServerHistoryTab({ run, sessionId, poller }: ServerHistoryTabProps) {
  ...
  const [records, setRecords] = useState<ServerBuild[] | null>(null)
  ...
  useEffect(() => {
    ...
    run(sessionId, { op: 'list' }).then((r) => {
      if (!alive) return
      const list = (r && r.ok) ? ((r.servers as ServerItem[]) || []) : []
      setServers(list)
  ...
  useEffect(() => {
    ...
    if (!server) { setJobsLoading(false); return }
    setJobsLoading(true)
  ...
  useEffect(() => {
    ...
    setSearch('')
    const server = servers.find((s) => s.id === serverId)
    if (!server || !jobPath) { setLoading(false); return }
    setLoading(true)
    ...
    run(sessionId, { op: 'jobHistory', serverId, segments }).then((r) => {
      ...
      setLoading(false)
      if (r && r.ok) {
        setRecords((r.builds as ServerBuild[] | undefined) || [])
      } else {
        setRecords([])
  ...
  const statusText = (b: ServerBuild): string =>
    b.building ? t('historyPending') : (b.result || '—')
  const list = records || []
  ...
  const resultClass = (b: ServerBuild): string => {
  ...
  const descText = (b: ServerBuild): string => {
  ...
  const openLog = (b: ServerBuild): void => {
  ...
  const serverOptions: InlineSelectOption[] = useMemo(
    () => servers.map((s) => ({ id: s.id, label: s.name })),
    [servers],
  )
  const jobOptions: InlineSelectOption[] = useMemo(
    () => jobs.map((j) => ({ id: j.path, label: j.path })),
    [jobs],
  )

  return (
    <>
      {/* 服务器 / Job 下拉：与「发布」tab 同款内联下拉，可直接搜索 */}
      ...
      <div className="dshj-server-field">
        ...
        <div className="dshj-server-ctrl">
          <InlineSelect
            value={jobPath}
            placeholder={!serverId ? t('jobPlaceholder')
      ...
      {/* 「日志记录」标题 + 右侧模糊搜索框（对当前 Job 已加载的记录过滤） */}
      <div className="dshj-server-history-head">
        <div className="dshj-server-history-title">{t('serverHistoryList')}</div>
        <div className="dshj-server-history-search">
          <input
      ...
      {!serverId || !jobPath ? (
        <div className="dshj-empty">{t('serverHistorySelectJob')}</div>
      ) : loading ? (
        <div className="dshj-empty">
          <span className="dshj-spinner" />
          <div>{t('serverHistoryLoading')}</div>
        </div>
      ) : error ? (
        <div className="dshj-empty"><div className="dshj-err">{error}</div></div>
      ) : list.length === 0 ? (
        <div className="dshj-empty">{t('serverHistoryEmpty')}</div>
      ) : filtered.length === 0 ? (
        <div className="dshj-empty">{t('serverHistoryNoMatch')}</div>
      ) : (
        <>
            ...
            {paged.map((b) => (
              ...
              >
                <div className="dshj-history-head">
                  <div className="dshj-server-history-left">
                    <span className={'dshj-history-result ' + resultClass(b)}>{statusText(b)}</span>
                    <span className="dshj-server-history-name">{nameText(b)}</span>
                  </div>
                  <span className="dshj-history-time">{fmtTime(b.timestamp)}</span>
                </div>
                <div className="dshj-server-history-desc">{descText(b)}</div>
              </button>
            ))}
          </div>
