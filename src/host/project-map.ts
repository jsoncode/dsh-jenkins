...
/** 项目配置文件绝对路径。 */
export function mapFilePath(dir: string): string {
  return join(dir, MAP_FILE)
}

/** 序列化为文件内容（2 空格缩进 + 末尾换行，便于人工编辑与 diff）。 */
export function serializeProjectMap(map: ProjectConfigMap): string {
  return JSON.stringify(map ?? {}, null, 2) + '\n'
}

/** 读取项目配置文件。不存在 → {}；损坏 → 备份 .bak 并返回 {}。 */
export async function loadProjectMap(dir: string): Promise<ProjectConfigMap> {
...
function doSave(dir: string, map: ProjectConfigMap): Promise<void> {
  return (async () => {
...
export function saveProjectMap(dir: string, map: ProjectConfigMap): Promise<void> {
