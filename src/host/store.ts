...
import { sanitizeProjectMap } from './projects.ts'
...
export const EMPTY_STORE = (): JenkinsStore => ({ version: STORE_VERSION, servers: [], cache: {} })
...
async function loadKey(dir: string): Promise<Buffer> {
  ...
  try { await chmod(keyPath, 0o600) } catch { /* Windows 尽力而为 */ }
  return key
}
...
function decryptToken(sealed: string, key: Buffer | null): string {
  if (!sealed) return ''
...
function sealStore(store: JenkinsStore, key: Buffer | null): string {
...
function openStore(raw: string, key: Buffer | null): LoadedStore {
  const parsed = JSON.parse(raw) as { version?: number; servers?: unknown; projects?: unknown; cache?: unknown }
  if (!parsed || typeof parsed !== 'object') throw new Error('store root must be an object')
  const servers = Array.isArray(parsed.servers)
    ? (parsed.servers as Array<Record<string, unknown>>).map((s) => ({
        id: String(s.id || ''),
        name: String(s.name || ''),
  ...
  const cache = parsed.cache && typeof parsed.cache === 'object' && !Array.isArray(parsed.cache)
    ? parsed.cache as Record<string, unknown>
    : {}
  // 旧版 `projects` 字段：仅作为迁移来源读出（非法项静默跳过，不影响服务器与缓存）。
  const legacyProjects = sanitizeProjectMap(parsed.projects)
  return { version: STORE_VERSION, servers, cache, legacyProjects }
}
 ...
 */
export async function loadStore(dir: string): Promise<LoadedStore | null> {
...
function doSave(dir: string, store: JenkinsStore): Promise<void> {
  return (async () => {
    await mkdir(dir, { recursive: true })
...
export function saveStore(dir: string, store: JenkinsStore): Promise<void> {
