 ...
 */
export async function parseConfigFromContent(shell: ShellService, filename: string, content: string): Promise<WorkspaceConfig & { file: string }> {
  ...
  if (name.endsWith('.json')) {
    ...
    const script =
      "import fs from 'node:fs';import os from 'node:os';import path from 'node:path';import {pathToFileURL as U} from 'node:url';" +
